...
int add_meeting(Diary* diary, Meeting* newMeeting){
	...
	if((int)(*diary->meetings[i]->begin_hour*100)%100 > 59 || (int)(*diary->meetings[i]->end_hour*100)%100 > 59) return 2;
...
int remove_meeting(Diary* diary, int index){
	int i;
	if(diary == NULL || index < 0) return 0;

	if(index == diary->num_of_meetings-1)
		destroy_Meeting(diary->meetings[index]);
	
	else
	{
		destroy_Meeting(diary->meetings[index]);

		while(index < diary->num_of_meetings-2)
			diary->meetings[index] = diary->meetings[index+1];

		destroy_Meeting(diary->meetings[index-1]);
	}
